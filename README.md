...
│   └── ConnectionQuality.cs
├── Resources/           # 图标与本地化字符串
│   ├── AkiLink.ico      # 应用图标（多尺寸 16/24/32/48/64/128/256，32bpp ARGB）
│   ├── AkiLink.png      # 源 PNG（1254×1254，透明背景）
│   ├── Locale.en-US.xaml
│   └── Locale.zh-CN.xaml
├── Services/            # 核心业务逻辑
│   ├── AudioVolumeService.cs        # Windows CoreAudio COM 封装
│   ├── BluetoothAudioService.cs     # WinRT AudioPlaybackConnection
│   ├── IAudioVolumeService.cs       # 音量服务接口
│   ├── IBluetoothAudioService.cs    # 蓝牙服务接口
│   ├── IDialogService.cs            # 对话框抽象（便于测试）
│   ├── INotificationService.cs      # 桌面通知抽象
│   ├── LocalizationService.cs       # 运行时语言切换
│   ├── SystemTrayService.cs         # 系统托盘图标 + 通知
├── Styles/              # XAML 资源与控制模板
│   ├── Brushes.xaml      # 调色板（深色 + 浅色）
│   ├── Controls.xaml     # 所有控件样式
│   └── ModernTheme.xaml  # 主题入口
...
│   ├── SettingsPanel.xaml
│   └── SettingsPanel.xaml.cs
├── tests/               # 单元测试（xUnit + Moq）
│   └── AkiLink.Tests/
│       ├── MainViewModelTests.cs   # 61 个测试
│       ├── SettingsServiceTests.cs # 6 个测试
│       └── IconGeometriesTests.cs  # 7 个测试
├── App.xaml             # 应用入口 + 资源
├── App.xaml.cs          # 启动 / 关闭编排
├── IconGeometries.cs    # 侧边栏图标 SVG 路径数据
├── MainWindow.xaml      # 主窗口布局
├── MainWindow.xaml.cs   # 窗口生命周期 + 最小化到托盘
